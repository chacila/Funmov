...
router.post('/ratings', async (req, res) => {
  ...
  let rating = await ratingRepo.findOneBy({ user_id: userId, movie_id: movieId });
  ...
  if (rating) {
    ...
    // Création
    rating = ratingRepo.create({ user_id: userId, movie_id: movieId, score: parsedScore });
    await ratingRepo.save(rating);
  }
 
  return res.status(200).json({ message: 'Note enregistrée.', movieId, score: parsedScore });
});
...
router.delete('/ratings/:movieId', async (req, res) => {
  ...
  const ratingRepo = appDataSource.getRepository(Rating);
  const result = await ratingRepo.delete({ user_id: userId, movie_id: movieId });
...
router.get('/recommendations', async (req, res) => {
  ...
  const recommendations = getRecommendations(userId, ratingsForPearson, moviesForPearson, limit);
